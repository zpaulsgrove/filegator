...
|---|----------|---------|----------|
| 1 | **High** | LDAP injection in login username → `ldap_search` filter | `backend/Services/Auth/Adapters/LDAP.php` |
| 2 | **High** | Vulnerable TOTP library `spomky-labs/otphp` 11.2.2 (2 advisories) | `composer.lock` |
| 3 | **Medium** | IDOR / predictable token: any user can download another user's batch archive | `backend/Controllers/DownloadController.php` |
| 4 | **Medium** | Upload reassembly uses non-namespaced filename in shared tmpfs (race / cross-user) | `backend/Controllers/UploadController.php` |
| 5 | **Medium** | Default admin ships with publicly-known credentials (`admin` / `admin123`) | `private/users.json.blank` |
| 6 | **Low** | Session cookie `Secure` flag off by default (`cookie_secure => null`) | `configuration_sample.php` |
| 7 | **Low** | User enumeration via password-reset response timing | `backend/Services/PasswordReset/PasswordResetService.php` |
| 8 | **Low** | Latent XSS: shared error toast renders server strings via Buefy `v-html` | `frontend/mixins/shared.js` |

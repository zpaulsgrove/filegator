...
* **MFA hardening pass:**
  ...
  * Fixed TOTP clock-drift tolerance: verification now allows the standard ±1 step (~30s) window instead of only ±1 second. OTPHP's `verify()` leeway argument is in seconds, not periods, so the previous literal `1` rejected otherwise-valid codes from users with mild clock skew or on requests that crossed a 30s window boundary. Replay protection still makes every accepted code single-use, so the reuse window is unchanged.
...
* **Frontend step-up dialog for admin sensitive actions:** the admin panel now opens a confirmation dialog before submitting `/storeuser`, `/updateuser`, `/deleteuser`, or `/admin/users/{u}/reset_mfa`. The dialog collects password + 6-digit code (or backup code) and forwards them as `stepup_*` fields. Always opens regardless of MFA state — the form adapts (password-only when no MFA enrolled). Self-service MFA flows in `/security` (disable, regenerate backup codes) now use the same shared form partial and benefit from inline field-error rendering (fixes a pre-existing "Verification failed" generic-toast swallow on wrong password/code). **Note:** the dialog's stolen-session re-auth defense is only enforced on adapters that implement `MfaCapableInterface` (`JsonFile` ships with this; LDAP and WPAuth do not). On non-MfaCapable adapters, the dialog appears and collects a password but the password is not verified server-side. See `docs/configuration/security.md` for details and the workaround.
